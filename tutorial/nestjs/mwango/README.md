...
アクセストークンの問題は、その有効期限が切れるまでは使用することができるため、誰かがアクセストークンを窃取した場合にはそのトークンを使用して認証を行うことができる。

またアクセストークンは有効期限が短いため、有効期限が切れるたびにログインをする必要が発生してしまい、ユーザーエクスペリエンスを低下させてしまう可能性が存在する。

そこで有効期限の長いリフレッシュトークンを別途作成し、アクセストークンの有効期限が切れた場合にはリフレッシュを行い、再度アクセストークンを発行することで、ユーザーはユーザー名やパスワードを再度入力する必要がなくなる。

またリフレッシュトークン自体も窃取された場合を考慮して、ログイン時にリフレッシュトークンをデータベースに保存して、誰かがリフレッシュを行った際にデータベースに保存されているトークンと比較をし、一致しない場合には拒否するようにする。

これで誰かのアクセスを拒否したい場合にはトークンを削除だけすればいい。

### 環境準備

まずはアクセストークンとリフレッシュトークン用の環境変数を以下のように準備する。

```bash
# .env.dev
JWT_ACCESS_TOKEN_SECRET=dev
JWT_ACCESS_TOKEN_EXPIRATION_TIME=900
JWT_REFRESH_TOKEN_SECRET=refresh_dev
JWT_REFRESH_TOKEN_EXPIRATION_TIME=86400
```

次にユーザーのデータモデルにリフレッシュトークン用のカラムを追加する。
