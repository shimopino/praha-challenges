...
- [ドメイン (モデル)](#%E3%83%89%E3%83%A1%E3%82%A4%E3%83%B3-%E3%83%A2%E3%83%87%E3%83%AB)
...
サブシステムやチームなどで境界が分けられることが多い。

シンプルに考える場合、1つの境界付けられたコンテキストに対して1アプリケーションになる、つまりはマイクロサービスアーキテクチャとなるイメージである。

コンテキスト同士の通信は、REST APIを採用して同期通信にしたり、AWS SQSなどのキューイングシステムを採用して非同期通信にしたりと色々と設計できる。

マイクロサービスアーキテクチャでなくとも、単純にパッケージの階層で区別をつけることも可能である。

## ドメイン (モデル)

ドメインモデルとは、ドメインの問題を解決するためのモデルであり、以下の要素で構成される。
...
ドメインサービスとは、モデルをオブジェクトをとして表現すると無理があるものに対する表現であり、集合に対する操作などが該当する。

例えば、メールアドレスの重複確認は、あるユーザーのオブジェクトでは解決できない課題であり、他のオブジェクトの状況に関する情報も必要となるため、ドメインサービスとして表現する

## リポジトリ

リポジトリは、集約単位で永続化層へのアクセスを提供するものである。

設計上では、リポジトリが有している集約をリストのようなものとして捉えることで、ユーザーを登録するなどのドメインロジックは追加することなく、ユーザーオブジェクトを追加したり、削除したりといった操作に集中することができる。

## アプリケーション (ユースケース)

アプリケーション (ユースケース) では、ドメインモデルが提供しているドメインロジックを組み合わせて、ユースケースを満たす処理を実現する。
