...
以下に悲観ロックと楽観ロックの違いをまとめる。

| ロックの種類 | 前提                                                                                                                                                       | 方法                                                                                          | 
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | 
| 楽観ロック   | ・事前に取得したレコードがほかのトランザクションによって改変されていないか確認する場合<br><br>・同一レコードに対する同時実行数が少なく、更新時間も短い場合 | ・テーブルにバージョン情報を管理するためのカラムを追加する                                    | 
| 悲観ロック   | ・処理を行う際にデータの整合性を行う必要がある場合<br><br>・同一レコードに対する同時実行数が多く、更新時間も長い場合                                       | ・悲観ロックを取得するための `SELECT` 文を発行する必要がある<br><br>・`FOR UPDATE` を使用する | 

楽観ロックでは、データに対してはロックを取得せずに、テーブルに追加したバージョン情報を使用して、データが最初に取得したときと同じ状態であるかを確認した後で更新することで、データの整合性を保つ手法である。

これはイメージとしては、以下のように `UPDATE` 文を発行する際に、レコードの主キーでの指定に加えてバージョン情報を検証することで、最初に取得したデータと同じ状態であるのかを検証している。

![](../assets/optimistics.png)

悲観ロックでは、更新対象のデータに対してロックを取得することで、他のトランザクションから更新されないようにすることで、データの整合性を保つ手法である。

これはイメージとしては、最初に更新対象のレコードを取得する際に `FOR UPDATE WAIT <seconds>` のようにロックを取得する処理を実行しておき、実際にロックが取得できるまでのトランザクション内の後続処理を待機させることで、データの整合性を保つようにしている。

![](../assets/pessimistics.png)
...
- [6.4. 排他制御¶](https://terasolunaorg.github.io/guideline/5.7.0.RELEASE/ja/ArchitectureInDetail/DataAccessDetail/ExclusionControl.html#id4)
